            ...
            <table width="500" cellpadding="0" cellspacing="0" border="0" class="container">
                <tr>
                    <td align="left" valign="top" class="desktop">
                        <h3 class="mail_header">Hello! {{$withdraw->user->name}}</h3>
                            <p style="font-size:15px !important;">You just withdrew <span style="font-weight:bold;">&#8358;{{ number_format($withdraw->amount, 2) }}</span> to account no: {{$user->accounts->account_no}}. 
                            You now have a balance of <span style="font-weight:bold;">&#8358;{{ number_format($withdraw->user->accounts->balance, 2) }}</span> in your wallet.</p>
